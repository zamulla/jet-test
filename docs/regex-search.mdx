...
And another using the `RegEx` constructor. This pattern uses the case-insensitive [option](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin.text/-regex-option/) to match words with capital letters as well.
...
All of the examples above are available on Github: https://github.com/zamulla/jet-test/blob/main/code-examples/regex.kt